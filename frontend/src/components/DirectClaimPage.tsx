  ...
  const checkInheritanceClaimable = async (claimRecord: ClaimData): Promise<boolean> => {
    ...
    try {
      ...
      if (claimRecord.type === 'sol') {
        ...
        const solHeir = await (program as any).account.solHeir.fetch(coinHeirPDA);
        const currentTime = Math.floor(Date.now() / 1000);
        const timeSinceActive = currentTime - solHeir.lastActivity.toNumber();
        
        return !solHeir.isClaimed && timeSinceActive > solHeir.inactivityPeriodSeconds.toNumber();
      } else {
        ...
        const tokenHeir = await (program as any).account.tokenHeir.fetch(tokenHeirPDA);
        const currentTime = Math.floor(Date.now() / 1000);
        const timeSinceActive = currentTime - tokenHeir.lastActivity.toNumber();
    ...
    try {
      ...
      if (claimData.type === 'sol') {
        ...
        const [solHeirPDA] = web3.PublicKey.findProgramAddressSync(
          [Buffer.from('sol_heir'), ownerPubkey.toBuffer(), heirPubkey.toBuffer()],
          program.programId
        );
        ...
        txSignature = await (program.methods as any)['claimSolInheritance']()
          .accountsPartial({
            solHeir: solHeirPDA,
            heir: heirPubkey,
