...
describe("gado", () => {
  ...
  it("Can perform batch token transfers", async () => {
    ...
    const recipients = [recipientKeypair.publicKey];
    const amounts = [new anchor.BN(1000000)]; // 1 token
  ...
  it("Can perform batch coin transfers", async () => {
    ...
    const recipients = [recipient.publicKey];
    const amounts = [new anchor.BN(LAMPORTS_PER_SOL / 100)]; // 0.01 SOL
    ...
    try {
      await program.methods
        ...
        .accounts({
          ...
          escrowTokenAccount: await getAssociatedTokenAddress(tokenMint, tokenHeirPda, true),
          tokenMint: tokenMint,
          tokenProgram: TOKEN_PROGRAM_ID,
        } as any)
        .signers([heir])
  ...
  it("Fails with too many batch transfers", async () => {
    const tooManyRecipients = Array(11).fill(Keypair.generate().publicKey);
    const tooManyAmounts = Array(11).fill(new anchor.BN(1000000));
    
    try {
      await program.methods
        .batchTransferTokens(tooManyRecipients, tooManyAmounts)
        .accounts({
          fromTokenAccount: ownerTokenAccount,
          authority: owner.publicKey,
          tokenProgram: TOKEN_PROGRAM_ID,
  ...
  it("Can handle zero amounts in batch transfers", async () => {
    const recipients = [heir.publicKey];
